        ...
        for j = 1, #self.任务序列, 1 do
            if self.execute_every_x_cycle[j] > 0 and i % self.execute_every_x_cycle[j] == 0 then
                ...
                if return_code < 0 then
                    Win.Print("任务组：任务出错，中止运行，错误代码" .. tostring(return_code))
                    return return_code
                end
            end
        end
...
function 任务组:new(任务序列, 循环次数, 超时, execute_every_x_cycle)
    local instance = {}
    ...
    instance.execute_every_x_cycle = execute_every_x_cycle
    return instance
end
